...
class dcube_body {
	...
	function treeView( $dirNode, $level=0 ) {
		...
		$hist1Ds = $dirNode["hist1d"];
		if ( is_array( $hist1Ds ) ) {
			foreach ( $hist1Ds as $nb => $hist1D ) {
				$this->histDiv( $hist1D, $level+1, $dirNode);
			}
		}
		
		$hist2Ds = $dirNode["hist2d"];
		if ( is_array( $hist2Ds ) ) {
			foreach ( $hist2Ds as $nb => $hist2D ) {
				$this->histDiv( $hist2D, $level+1, $dirNode);
			}
		}
		
		$graphs = $dirNode["graph"];
		if ( is_array( $graphs ) ) {
			foreach ( $graphs as $nb => $graph ) {
			$this->histDiv( $graph, $level+1, $dirNode);
			}
		}
	...
	function histDiv( $hist, $level, $parDir) {
		...
		if ( strstr($type, "TGraph") ) $icon = "graph";
		if ( strstr($type, "TProfile") ) $icon = "profile";

		if ( $level == 1 ) {
			echo "<div class='his' style='display: block;' title='".$histName."' ".
				"onmouseover=\"statusBar('".$type." ".$histName."');\" onclick=\"".$showhide."\">";
		}  else {
			echo "<div class='his' style='display: auto;' title='".$histName."' ".
				"onmouseover=\"statusBar('".$type." ".$histName."');\" onclick=\"".$showhide."\">";
		}
	...
	function histContentsDIV( $histNode, $id ) {
		...
		if ( strlen($status) ) {
			...
			echo "<div class='tp'>";
			echo "<img class='plot' alt='plot' id='".$pid."' src='".$this->where."./images/loading_plot.gif'>";
			echo "</div>";		
	...
	function plotView( $object, $dirname , $pid ) {
	  ...
	  for ($i = 0; $i < $this->plotsPerPage; $i++ ) {
	    echo "<div class='gp' id='gp_".($i+0)."'><div class='gs' id='gs_".($i+0)."'></div>";
	...
	function statbox( $object, $type ) {
	  ...
	  $obj = $object["stat"][0];
		...
		if ( strstr($type, "TGraph" ) ) {

			$p = $obj["points"][0]["cdata"] + 0;
			$rp = $obj["points"][0]["attr"]["ref"][0] + 0;

			$m = $obj["mean"][0]["cdata"] + 0;
			$rm = $obj["mean"][0]["attr"]["ref"] + 0;
			
			$r = $obj["rms"][0]["cdata"] + 0;
			$rr = $obj["rms"][0]["attr"]["ref"] + 0;
